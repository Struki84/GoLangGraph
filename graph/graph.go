...
type Node struct {
	...
	Function func(ctx context.Context, state MessagesState) (MessagesState, error)
}
...
type Edge struct {
	...
	To func(ctx context.Context, state MessagesState) string
}
...
func NewMessagesState(messages []llms.MessageContent) MessagesState {
...
func (g *MessageGraph) AddNode(name string, fn func(ctx context.Context, state MessagesState) (MessagesState, error)) {
	g.nodes[name] = Node{
		Name:     name,
		Function: fn,
	}
...
func (g *MessageGraph) AddEdge(from, to string) {
	g.edges = append(g.edges, Edge{
		From: from,
		To: func(_ context.Context, state MessagesState) string {
...
func (g *MessageGraph) AddConditionalEdge(from string, condition func(ctx context.Context, state MessagesState) string) {
	g.edges = append(g.edges, Edge{
		From: from,
		To:   condition,
	})
...
// Invoke executes the compiled message graph with the given input messages.
// It returns the resulting messages and an error if any occurs during the execution.
func (r *Runnable) Invoke(ctx context.Context, messages MessagesState) (MessagesState, error) {
	state := messages
	...
	for {
		...
		if !ok {
			return MessagesState{}, fmt.Errorf("%w: %s", ErrNodeNotFound, currentNode)
		}

		var err error
		state, err = node.Function(ctx, state)
		if err != nil {
			return MessagesState{}, fmt.Errorf("error in node %s: %w", currentNode, err)
		}
		...
		if !foundNext {
			return MessagesState{}, fmt.Errorf("%w: %s", ErrNoOutgoingEdge, currentNode)
		}
	}
